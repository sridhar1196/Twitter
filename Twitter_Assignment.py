...
cou = []
file = open('twitter_3.txt','r', encoding='utf-8')
ids = {}
hrs = {}
for line in file:
    list1 = line.split()

    acc = Account(list1[0],list1[1][1:15],int(list1[-2]), int(list1[-1]),1)
    ac = Account(list1[0],list1[1][1:15],int(list1[-2]), int(list1[-1]),1)
...
mostuser = open("ans1.txt","w",encoding="utf-8")
mosthour = open("ans2.txt","w",encoding="utf-8")
mostfollowers = open("ans3.txt","w",encoding="utf-8")
mosttweets = open("ans4.txt","w",encoding="utf-8")
...
for x in range(0,numbertoconsider):
    mostfollowers.write(max_followers[x].fetch_tweet_id()+ ", Account Followers:"+  str(max_followers[x].fetch_followers())+"\n")
    mostfollowers.flush()    
